      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {plans.map((plan) => <article id={plan.monitor ? "monitor-plan" : undefined} key={plan.name} className={`bg-white p-8 lg:p-10 relative border scroll-mt-32 ${plan.monitor ? "border-[#a88750] bg-[#fcf8ef] lg:scale-[1.035] z-10 shadow-[0_18px_50px_rgba(83,63,30,0.12)]" : plan.recommended ? "border-[var(--fukai-ai)]" : "border-[var(--usuzumi-line)]"}`}>
            ...
            <h3 className="heading-md text-lg mb-5 pr-16">{plan.name}</h3>{plan.regularPrice && <p className="mb-1 text-xs tracking-wide text-[var(--nibi)]">通常 <span className="line-through decoration-[#a88750] decoration-2">{plan.regularPrice}</span></p>}<p className={`${plan.monitor ? "text-3xl text-[#806334]" : "text-2xl"} font-medium mb-8`}>{plan.price}</p><FeatureList features={plan.features} />
