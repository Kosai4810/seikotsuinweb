...
export function ArticlePage({ article }: { article: Article }) {
  return <><Header /><main className="bg-[#f8f7f4]"><article>
    ...
    <div className="max-w-4xl mx-auto px-6 py-14 lg:py-20">
      ...
      <aside className="mt-20 lg:mt-28 bg-[var(--sumi)] px-7 py-10 lg:px-12 lg:py-12 text-white"><h2 className="font-serif text-2xl leading-relaxed">読んだあと、院に合う形を一緒に整理できます。</h2><p className="mt-4 text-sm leading-7 text-white/65">まだ依頼内容が決まっていなくても問題ありません。必要なものと、今は要らないものから整理します。</p><div className="mt-7 flex flex-col sm:flex-row gap-3"><Link href={article.relatedHref} className="inline-flex justify-center bg-white px-6 py-4 text-sm text-[var(--sumi)] hover:bg-[#f0ede7]">{article.relatedLabel}</Link><Link href="/#contact" className="inline-flex justify-center border border-white/30 px-6 py-4 text-sm text-white hover:bg-white/10">無料相談を申し込む</Link></div></aside>
