...
export function SeoLandingPage({ eyebrow, title, lead, image, imageAlt, sections, relatedHref, relatedLabel }: { eyebrow?: string; title: string; lead: string; image: string; imageAlt: string; sections: Section[]; relatedHref: string; relatedLabel: string }) {
  return <>
    <Header />
    <main>
      <section className="pt-32 md:pt-40 pb-20 md:pb-28 bg-[#171512] text-white">
        <div className="max-w-7xl mx-auto px-6 lg:px-12 grid lg:grid-cols-2 gap-12 items-center">
          <div>{eyebrow && <p className="mb-4 text-[11px] font-bold tracking-[.2em] text-[#d2b47c]">{eyebrow}</p>}<h1 className="font-serif text-3xl md:text-5xl leading-snug tracking-wide mb-7">{title}</h1><p className="text-sm md:text-base text-white/75 leading-loose mb-8">{lead}</p><Link href="/#contact-form" className="inline-flex bg-[#a88750] text-white px-7 py-4 text-sm">無料HP診断を受ける</Link></div>
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-12 py-20 md:py-28">
        ...
        <div className="mt-14 text-center"><h2 className="heading-lg text-2xl md:text-3xl mb-6">まずは現在の状況をお聞かせください。</h2><p className="text-sm text-[var(--sumi-nezumi)] mb-7">まだ内容が決まっていなくても問題ありません。必要な準備と優先順位から整理します。</p><Link href="/#contact-form" className="inline-flex bg-[#a88750] text-white px-8 py-4 text-sm">無料HP診断を受ける</Link></div>
      </div>
    </main>
    <Footer /><MobileCTA />
  </>;
