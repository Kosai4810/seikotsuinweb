...
export function EngagementLayer() {
  ...
  return (
    <>
      ...
      <aside aria-label="料金プランと無料HP診断のご案内" aria-hidden={!visible} className={`pricing-popup fixed left-1/2 top-1/2 z-[60] w-[min(calc(100vw-1.5rem),390px)] max-h-[min(78svh,560px)] -translate-x-1/2 overflow-y-auto rounded-xl border border-[#c9b795] bg-[#fbf8f1] p-5 shadow-[0_24px_90px_rgba(48,38,23,0.24)] transition-all duration-300 ${visible ? "-translate-y-1/2 scale-100 opacity-100" : "pointer-events-none -translate-y-[44%] scale-[0.97] opacity-0"}`}>
        ...
        <p className="mt-3 text-xs leading-5 text-[#665c4c]">既存HPがある院は改善点を無料診断。HPがない院は必要な導線を整理します。</p>
        <div className="mt-4 grid grid-cols-2 gap-2">
          <a href="#contact-form" onClick={() => setVisible(false)} className="inline-flex items-center justify-center rounded-md bg-[#806334] px-3 py-3 text-center text-xs font-medium text-white">無料診断する</a>
